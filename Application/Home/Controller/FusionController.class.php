...
class FusionController extends Controller {
    ...
    public function ajaxGetMinuteChartData(){
        ...
        $hour_array = array();
        $minute_array = array();
        $fast_percent_num_array = array();
        $slow_percent_num_array = array();
        $fast_tooltext_array = array();
        $slow_tooltext_array = array();
        $fast_true_num_array = array();
        $slow_true_num_array = array();
        while(!feof($fp)) {
            $row = trim(fgets($fp));
            if ($row != "") {
                $info = explode(",", $row);
                if(count($info) == 3){
                    ...
                    $fast_true_num_array[] = (intval(($info[1]) / 10)*10);
                    $slow_true_num_array[] = (intval(($info[2]) / 10)*10);
        ...
        for($i=0;$i<$num;$i++){
            $tmp = array();
            $tmp['value'] = $fast_percent_num_array[$i];
        ...
        for($i=0;$i<$num;$i++){
            $tmp = array();
            $tmp['value'] = $slow_percent_num_array[$i];
        ...
        $responce['caption'] = "统计: [快线   " . $fast_data[count($fast_data) - 1]['tooltext'] . " ]   [慢线   " . $slow_data[count($slow_data) - 1]['tooltext'] . " ]";
        //trace($responce);
